...
class NumDataset(Dataset):
    ...
    #     inputs = self.tokenizer(self.X[idx], return_tensors="pt", padding="max_length", truncation=True, max_length=self.max_length)
    #     labels = self.y[idx]
...
model = TransformerRegressor("bert-base-uncased", 1)
...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        outputs = model(input_ids, attention_mask).squeeze() # 入力を[16]に変換
        loss = criterion(outputs, labels)
        loss.backward()
        optimizer.step()
